...
nextAction: Design review batch 3 (wheel T4, T14-T18) on branch design/granskning, see top section
blockers: []
...
- **Batch 3 påbörjad**: rena funktioner `springLinear`, `SPIN`, `SPIN_MS`, `spinAngle`, `landingTravel`, `sectorAtAngle`, `WHEEL_COLORS` i app.js med tester. `spinWheel` använder tillfälligt `landingTravel` i gamla WAAPI-snurret. Utkast till ny hjulvy (block A: markup, panel, legend, läsfönster) i `design_handoff_sipdeck/wheel-view-draft.js`, ej inlagt. Kvar: T4, T14–T18 enligt plan nedan.
- **Fynd:** T16:s krav "derivatan ≤ 1000°/s" håller inte vid maxresan: 1 062°/s vid 4 varv + 359°, 957 vid typisk resa. Testet mäter typisk resa. Ägarbeslut om kurvan ska ändras.
- **Plan batch 3:** hjulet renderas i eget lager `#wheelLayer` ovanpå kortleken (kortleken ligger kvar i `#view`, så FLIP öppna/stäng kan skala startsidan och stängningen aldrig klipper); `render()` hoppar över `#view` när basen redan är kortleken och lagret är öppet. Läge väljs med fem knappar (`data-wheel-mood`); att välja aktivt läge igen = nytt hjul (ersätter "Nytt hjul"). Resultatkortet ersätter väljaren, "Byt läge · {läge}" visar den igen. Ny inställning `wheelLabels` (normalize/reconcile). Minihjulet i `index.html` behåller statisk SVG, färgas per kategori. Flyg bara `#wheelDisc` (rotationen måste ingå i keyframes), mät på `#wheelStage`. Test.js-källkontroller för View Transitions/fallback/`wheel-result-actions`/`default to first mood` ska skrivas om.
- Verktyg: port 4173 upptas av annat projekt; kör `python -m http.server 4199` och `BASE_URL=http://127.0.0.1:4199 npx playwright test -c img-src/verify.config.cjs`. Hjälpskript `img-src/shot.cjs`, `img-src/drag.cjs` (gitignorerade). app.js-budget 99 kB. dcg-hooken blockerar heredoc med `>` i Bash: använd Edit eller skriptfil.
- Kvar efter batch 3: batch 4 (fas 4), batch 5 (F1 varianter), batch 6 (F2/F3 Worker+D1, stoppa och fråga före deploy/migration), F4 (48 bilder via Codex CLI `codex`, finns installerad).
