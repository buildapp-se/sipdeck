...
nextAction: Owner approval for batch 6 remote steps (D1 schema, Worker deploy, ADMIN_TOKEN secret), then F4 generic images; see top section
blockers: []
...
- **Batch 6, ej gjort (kräver ägarens ja, Sipdeck har riktiga användare):** (1) `cd worker && npx wrangler d1 execute sipdeck --remote --file schema.sql` (idempotent, `IF NOT EXISTS`, några få skrivna rader). (2) `cd worker && npx wrangler deploy`. (3) `npx wrangler secret put ADMIN_TOKEN` (minst 32 tecken, spara lokalt, aldrig i noter). (4) Ordning: 1 före 2, annars ger nya rutter 500. Förbrukning läst 2026-09-25: kontots D1 ca 2 500 skrivna och 88 000 lästa rader senaste dygnet (2,5 % resp. 1,8 %). Därefter: merge till main och Pages-deploy av frontenden, som alltid.
...
- Kvar: F4 (48 bilder via Codex CLI `codex`, finns installerad), sedan byter `artMarkup` silhuetten mot `img-generic/<glas>-<färg>.webp` för egna drinkar.
