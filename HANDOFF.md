...
- Verktyg: port 4173 upptas av annat projekt; kör `python -m http.server 4199` och `BASE_URL=http://127.0.0.1:4199 npx playwright test -c img-src/verify.config.cjs`. Hjälpskript (gitignorerade): `img-src/shot.cjs`, `img-src/drag.cjs`, `img-src/wheelshot.cjs <url> <dir>` (hjulets lägen i 390×844/664, ljust/mörkt, `LABELS=1` för sektornamn, skriver panelens nederkant), `img-src/flipshot.cjs <url> <dir>` (bildrutor mitt i öppna/stäng). app.js-budget 120 kB. dcg-hooken blockerar heredoc med `>` och `rm -rf` i Bash: använd Edit, skriptfil eller en ny mapp.
