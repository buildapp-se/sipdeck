...
nextAction: Design review batch 5 (F1 varianter) on branch design/granskning, see top section
blockers: []
...
- Verktyg: port 4173 upptas av annat projekt; kör `python -m http.server 4199` och `BASE_URL=http://127.0.0.1:4199 npx playwright test -c img-src/verify.config.cjs`. Hjälpskript (gitignorerade): `img-src/shot.cjs`, `img-src/drag.cjs`, `img-src/wheelshot.cjs <url> <dir>` (hjulets lägen i 390×844/664, ljust/mörkt, `LABELS=1` för sektornamn, skriver panelens nederkant), `img-src/flipshot.cjs <url> <dir>` (bildrutor mitt i öppna/stäng). app.js-budget 104 kB. dcg-hooken blockerar heredoc med `>` och `rm -rf` i Bash: använd Edit, skriptfil eller en ny mapp.
- Kvar efter batch 4: batch 5 (F1 varianter), batch 6 (F2/F3 Worker+D1, stoppa och fråga före deploy/migration), F4 (48 bilder via Codex CLI `codex`, finns installerad).
