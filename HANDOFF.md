...
- Kvar: (1) mejlavisering vid nytt förslag (ägarbeslut 2026-09-25): Workern mejlar patz.lofgren@gmail.com via Cloudflare `send_email` när `POST /suggestions` lyckas, i `ctx.waitUntil` så att ett misslyckat mejl aldrig stoppar förslaget; kräver Email Routing på en zon (troligen buildapp.se) och verifierad mottagare, redeploy av Workern efter ägarens ja. (2) F4 (48 bilder via Codex CLI `codex`, finns installerad), sedan byter `artMarkup` silhuetten mot `img-generic/<glas>-<färg>.webp` för egna drinkar.
